    ...
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-slate-900/60 p-4 rounded-2xl border border-slate-800 backdrop-blur-md">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-gradient-to-tr from-cyan-500/20 to-blue-500/20 border border-cyan-500/30 flex items-center justify-center text-cyan-400 shadow-lg shadow-cyan-950">
        ...
        <div className="flex items-center gap-2.5 w-full sm:w-auto justify-end">
          <button
            ...
            className="px-3 py-1.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-300 text-xs font-medium border border-slate-700 transition-all flex items-center gap-1.5 cursor-pointer disabled:opacity-50"
          ...
          <button
            ...
            disabled={isAuditing}
            className="px-3.5 py-1.5 rounded-xl bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-600 hover:from-blue-500 hover:to-purple-500 text-white text-xs font-semibold shadow-lg shadow-indigo-950 border border-indigo-500/30 transition-all flex items-center gap-1.5 cursor-pointer disabled:opacity-50"
          >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        {/* Download Speed */}
        <div className="bg-slate-900/80 border border-slate-800/80 rounded-2xl p-4 flex flex-col justify-between relative overflow-hidden group hover:border-cyan-500/40 transition-all">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs font-medium text-slate-400">Tốc độ Tải Xuống (RX)</span>
        ...
        {/* Upload Speed */}
        <div className="bg-slate-900/80 border border-slate-800/80 rounded-2xl p-4 flex flex-col justify-between relative overflow-hidden group hover:border-emerald-500/40 transition-all">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs font-medium text-slate-400">Tốc độ Tải Lên (TX)</span>
        ...
        <div className="bg-slate-900/80 border border-slate-800/80 rounded-2xl p-4 flex flex-col justify-between relative overflow-hidden group hover:border-blue-500/40 transition-all">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs font-medium text-slate-400">Đang Ra Internet</span>
          ...
          <div className="flex items-baseline gap-1.5">
            ...
            <span className="text-xs text-slate-400">kết nối</span>
          </div>
          <div className="mt-2 text-[11px] text-slate-500 flex items-center gap-1">
        ...
        <div className="bg-slate-900/80 border border-slate-800/80 rounded-2xl p-4 flex flex-col justify-between relative overflow-hidden group hover:border-amber-500/40 transition-all">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs font-medium text-slate-400">Cổng Dịch Vụ Mở</span>
          ...
          <div className="flex items-baseline gap-1.5">
            ...
            <span className="text-xs text-slate-400">cổng</span>
          </div>
          <div className="mt-2 text-[11px] text-slate-500 flex items-center gap-1">
      ...
      <div className="bg-slate-900/70 border border-slate-800/80 rounded-2xl p-5 backdrop-blur-sm">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-semibold text-white">Card Mạng Hệ Thống (Network Interfaces)</h3>
            <span className="text-xs text-slate-500">
              (Gateway: <span className="font-mono text-slate-300">{overview?.defaultGateway || '192.168.1.1'}</span>)
            </span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
          {overview?.interfaces.map((iface) => (
            <div
              key={iface.name}
              className="p-3.5 bg-slate-950/70 border border-slate-800/90 rounded-xl hover:border-slate-700 transition-all flex flex-col justify-between"
            >
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  <span className="text-xs font-bold text-white font-mono">{iface.name}</span>
                  {iface.internal ? (
                    <span className="px-1.5 py-0.5 rounded text-[10px] bg-slate-800 text-slate-400">Loopback</span>
                  ) : iface.virtual ? (
                    <span className="px-1.5 py-0.5 rounded text-[10px] bg-indigo-500/10 text-indigo-400 border border-indigo-500/20">Virtual</span>
                  ) : (
                    <span className="px-1.5 py-0.5 rounded text-[10px] bg-cyan-500/10 text-cyan-400 border border-cyan-500/20">Physical</span>
                  )}
                </div>
                <span
                ...
                >
                  <span
                    className={`w-1.5 h-1.5 rounded-full ${
                      iface.operstate === 'up' ? 'bg-emerald-400 animate-pulse' : 'bg-rose-400'
                    }`}
                  />
              ...
              <div className="space-y-1 text-[11px] font-mono text-slate-300">
                <div className="flex justify-between">
                  ...
                  <span className="text-cyan-300 font-medium">{iface.ip4}</span>
                </div>
                <div className="flex justify-between">
      ...
      <div className="bg-slate-900/70 border border-slate-800/80 rounded-2xl p-5 backdrop-blur-sm space-y-4">
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2.5 pt-2">
          ...
          {/* Tabs bộ lọc hướng */}
          <div className="flex items-center bg-slate-950 p-1 rounded-xl border border-slate-800 text-[11px] overflow-x-auto">
            <button
          ...
          <div className="relative">
            <select
              ...
              className="bg-slate-950 border border-slate-800 rounded-xl px-3 py-1.5 text-xs text-slate-300 focus:outline-none focus:border-cyan-500/50 cursor-pointer appearance-none pr-8"
            >
        ...
        <div className="border border-slate-800 rounded-xl overflow-hidden bg-slate-950">
          <div className="overflow-x-auto max-h-[460px] overflow-y-auto">
            <table className="w-full text-left text-xs">
              <thead className="bg-slate-900/90 text-slate-400 text-[11px] uppercase tracking-wider sticky top-0 z-10 border-b border-slate-800">
                <tr>
              ...
              <tbody className="divide-y divide-slate-800/60 font-mono text-[11px]">
                {filteredConnections.length === 0 ? (
                  ...
                  filteredConnections.map((conn) => {
                    ...
                    return (
                      <tr key={conn.id} className="hover:bg-slate-900/50 transition-colors">
                        {/* Process & PID */}
                        ...
                        <td className="py-2.5 px-3 text-slate-300">
                          <div>
                            {conn.localAddress}
                            <span className="text-amber-400 font-bold">:{conn.localPort}</span>
                          </div>
                        </td>
                          ...
                          {isListen ? (
                            ...
                            <div>
                              <div className="text-cyan-300 font-medium">
                                ...
                                <span className="text-cyan-400 font-bold">:{conn.peerPort}</span>
                              </div>
                        ...
                        <td className="py-2.5 px-3 font-sans">
                          <div className="flex flex-col gap-0.5">
                            <span className="text-slate-200 font-medium text-[11px]">{conn.serviceName}</span>
                            {isListen ? (
                        ...
                        <td className="py-2.5 px-3 text-right">
                          {conn.pid && conn.pid > 1 ? (
                            <button
                              ...
                              className="px-2 py-1 rounded-lg bg-rose-500/10 hover:bg-rose-500/20 text-rose-400 border border-rose-500/20 hover:border-rose-500/40 text-[10px] font-sans font-medium transition-all cursor-pointer disabled:opacity-50 inline-flex items-center gap-1"
                            >
                              <XCircle className="w-3 h-3" />
                              <span>{killingPid === conn.pid ? 'Đang dừng...' : 'Ngắt kết nối'}</span>
                            </button>
                          ) : (
                            <span className="text-[10px] text-slate-600 font-sans">Bảo vệ</span>
                          )}
                        </td>
                      </tr>
                    );
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {/* Ping Test */}
        <div className="bg-slate-900/70 border border-slate-800/80 rounded-2xl p-4 backdrop-blur-sm space-y-3">
          <div className="flex items-center gap-2">
            <Radio className="w-4 h-4 text-cyan-400" />
            <h4 className="text-xs font-semibold text-white">Kiểm Tra Độ Trễ Mạng (Ping Test)</h4>
          </div>
          <form onSubmit={handlePingTest} className="flex gap-2">
            <input
              ...
              className="flex-1 bg-slate-950 border border-slate-800 rounded-xl px-3 py-1.5 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-cyan-500/50 font-mono"
            />
            <button
              type="submit"
              disabled={pingLoading}
              className="px-3.5 py-1.5 rounded-xl bg-cyan-600 hover:bg-cyan-500 text-white text-xs font-medium transition-all flex items-center gap-1 cursor-pointer disabled:opacity-50"
            >
          ...
          {pingResult && (
            <div className="p-3 bg-slate-950 rounded-xl border border-slate-800 text-xs font-mono space-y-1">
              <div className="flex justify-between items-center">
                <span className="text-slate-400">Đích đến: {pingResult.target}</span>
                {pingResult.success ? (
                  ...
                  <span className="text-rose-400 font-bold">Thất bại</span>
                )}
              </div>
              {pingResult.rawOutput && (
                <pre className="text-[10px] text-slate-400 overflow-x-auto max-h-20 whitespace-pre-wrap pt-1 border-t border-slate-800/80">
                  {pingResult.rawOutput}
                </pre>
              )}
            </div>
        ...
        <div className="bg-slate-900/70 border border-slate-800/80 rounded-2xl p-4 backdrop-blur-sm space-y-3">
          <div className="flex items-center gap-2">
            ...
            <h4 className="text-xs font-semibold text-white">Tra Cứu Phân Giải Tên Miền (DNS Lookup)</h4>
          </div>
          <form onSubmit={handleDnsLookup} className="flex gap-2">
            <input
              ...
              className="flex-1 bg-slate-950 border border-slate-800 rounded-xl px-3 py-1.5 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-emerald-500/50 font-mono"
            />
            <button
              type="submit"
              disabled={dnsLoading}
              className="px-3.5 py-1.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-medium transition-all flex items-center gap-1 cursor-pointer disabled:opacity-50"
            >
          ...
          {dnsResult && (
            <div className="p-3 bg-slate-950 rounded-xl border border-slate-800 text-xs font-mono space-y-1">
              <div className="text-slate-400">Tên miền: {dnsResult.domain}</div>
              {dnsResult.success ? (
                <div>
                  <div className="text-emerald-400 text-[11px] font-bold">Bản ghi IPv4 (A):</div>
                  <div className="text-slate-300 text-[11px] pl-2">
                    {dnsResult.ipv4.length > 0 ? dnsResult.ipv4.join(', ') : 'Không có'}
                  </div>
                  {dnsResult.ipv6.length > 0 && (
                    <>
                      <div className="text-cyan-400 text-[11px] font-bold mt-1">Bản ghi IPv6 (AAAA):</div>
                      <div className="text-slate-300 text-[11px] pl-2">{dnsResult.ipv6.join(', ')}</div>
                    </>
                  )}
                </div>
              ) : (
                <div className="text-rose-400">{dnsResult.error}</div>
              )}
            </div>
          )}
        </div>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-2xl overflow-hidden shadow-2xl flex flex-col max-h-[85vh]">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-800 bg-slate-900/60">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h3 className="text-sm font-semibold text-white">AI Phân Tích An Ninh Mạng (Gemini 3.8 Flash)</h3>
                  <p className="text-[11px] text-slate-400">Kiểm tra kết nối ra ngoài, cổng mở và lỗ hổng bảo mật</p>
                </div>
              </div>
              <button
            ...
            <div className="p-6 overflow-y-auto space-y-4">
              {isAuditing ? (
                <div className="py-12 flex flex-col items-center justify-center gap-3">
                  ...
                  <p className="text-sm text-slate-300 font-medium">Trợ lý AI đang rà soát từng kết nối Internet và cổng mở...</p>
                  <p className="text-xs text-slate-500">Phân tích rò rỉ dữ liệu, tiến trình lạ và an toàn tường lửa</p>
                </div>
              ) : auditResult ? (
                <div className="space-y-4">
                  ...
                  <div className="p-4 rounded-xl bg-slate-950 border border-slate-800 flex items-center justify-between">
                    <div>
                      <div className="text-xs text-slate-400">Điểm Đánh Giá An Toàn Mạng</div>
                    ...
                    <div className="p-4 rounded-xl bg-slate-950 border border-slate-800 space-y-2">
                      ...
                      <div className="space-y-1 font-mono text-[11px]">
                        {auditResult.suggestedCommands.map((cmd, idx) => (
                          <div key={idx} className="p-2 bg-slate-900 rounded-lg text-cyan-300 border border-slate-800/80">
                            {cmd}
                          </div>
                        ))}
                      </div>
            ...
            <div className="flex items-center justify-end gap-2 px-6 py-3 border-t border-slate-800 bg-slate-900/50">
              <button
                onClick={() => setShowAuditModal(false)}
                className="px-4 py-2 rounded-xl text-xs font-medium bg-slate-800 hover:bg-slate-700 text-white transition-colors cursor-pointer"
              >
                Đóng
              </button>
            </div>
