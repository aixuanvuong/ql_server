...
export const AgentView: React.FC<AgentViewProps> = ({
  ...
  return (
    <div className="space-y-4 pb-20 md:pb-6">
      ...
      <div className="bg-gradient-to-r from-purple-950/80 via-slate-900 to-slate-900 border border-purple-500/30 rounded-2xl p-4 sm:p-5 shadow-lg flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-3.5">
          <div className="w-12 h-12 rounded-2xl bg-purple-500/20 border border-purple-500/40 text-purple-300 flex items-center justify-center flex-shrink-0 shadow-inner">
        ...
        {metrics && (
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 bg-slate-950/70 p-2.5 rounded-xl border border-slate-800 text-[11px] font-mono">
            <div className="flex items-center gap-1.5 px-2">
              <Cpu className="w-3.5 h-3.5 text-emerald-400" />
              <span className="text-slate-400">CPU:</span>
              <span className="text-emerald-300 font-bold">{metrics.cpu.loadPercent}%</span>
            </div>
            <div className="flex items-center gap-1.5 px-2">
              <Database className="w-3.5 h-3.5 text-purple-400" />
              <span className="text-slate-400">RAM:</span>
              <span className="text-purple-300 font-bold">{metrics.memory.usedPercent}%</span>
            </div>
            <div className="flex items-center gap-1.5 px-2">
              <Flame className="w-3.5 h-3.5 text-amber-400" />
              <span className="text-slate-400">Nhiệt:</span>
              <span className="text-amber-300 font-bold">{metrics.cpu.temperature}°C</span>
            </div>
            <div className="flex items-center gap-1.5 px-2">
