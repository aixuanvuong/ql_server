      ...
      <div className="bg-slate-900 border border-slate-700/80 rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden flex flex-col max-h-[92vh]">
        ...
        <form onSubmit={handleSave} className="p-4 sm:p-6 overflow-y-auto space-y-5 flex-1">
          ...
          <div className="space-y-1.5">
            ...
            <div className="bg-slate-950 border border-slate-800 rounded-xl px-3.5 py-2.5 text-xs text-purple-300 font-mono flex items-center justify-between">
              <span className="truncate">{config?.baseURL || 'https://omniroute.xuanvuong.id.vn/v1'}</span>
              <span className="text-[10px] bg-slate-800 text-slate-400 px-2 py-0.5 rounded">Cố định</span>
            </div>
          </div>
            ...
            {!isCustomMode ? (
              ...
              <div className="space-y-1.5">
                <input
                  ...
                  className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3.5 py-2.5 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-blue-500 transition-colors font-mono"
                />
          ...
          <div className="space-y-1.5">
            ...
            <input
              ...
              className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3.5 py-2.5 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-amber-500 transition-colors font-mono"
            />
            <p className="text-[11px] text-slate-500">
              Khóa API sẽ được mã hóa và lưu trực tiếp vào file <code className="text-amber-400/80">.env</code> trên máy chủ.
            </p>
          </div>
          ...
          <div className="pt-2 border-t border-slate-800 flex flex-wrap items-center justify-between gap-2.5">
            <button
              ...
              className="px-3.5 py-2 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700 text-xs font-medium flex items-center gap-1.5 transition-colors cursor-pointer disabled:opacity-50"
            >
            ...
            <div className="flex items-center gap-2">
              <button
                type="button"
                onClick={onClose}
                className="px-3.5 py-2 rounded-xl bg-slate-800/80 hover:bg-slate-800 text-slate-300 text-xs font-medium transition-colors cursor-pointer"
              >
                Hủy
              </button>
              <button
                type="submit"
                disabled={saving}
                className="px-4 py-2 rounded-xl bg-purple-600 hover:bg-purple-500 text-white text-xs font-semibold shadow-lg shadow-purple-600/30 flex items-center gap-1.5 transition-all cursor-pointer disabled:opacity-50"
              >
                {saving ? (
