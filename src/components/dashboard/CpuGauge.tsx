...
export const CpuGauge: React.FC<CpuGaugeProps> = ({
  ...
  const getTempStatus = (temp: number) => {
    if (temp < 55) {
      return { text: 'Mát mẻ', color: 'text-emerald-700 dark:text-emerald-400 bg-emerald-500/10 border-emerald-500/30' };
    }
    if (temp < 75) {
      return { text: 'Bình thường', color: 'text-amber-700 dark:text-amber-400 bg-amber-500/10 border-amber-500/30' };
    }
    return { text: 'Cảnh báo nóng', color: 'text-rose-700 dark:text-rose-400 bg-rose-500/10 border-rose-500/30' };
  };
    ...
    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 shadow-sm dark:shadow-lg transition-colors">
      <div className="flex items-center justify-between gap-2 mb-4">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="text-sm font-semibold text-slate-900 dark:text-white">Bộ vi xử lý (CPU)</h3>
            <p className="text-[11px] text-slate-500 dark:text-slate-400 truncate max-w-[200px] sm:max-w-xs">{cpuBrand}</p>
          </div>
        </div>
      ...
      <div className="mb-5">
        <div className="flex justify-between items-baseline mb-1.5">
          <span className="text-xs text-slate-500 dark:text-slate-400">Tải trung bình tất cả các nhân</span>
          <span className="text-lg font-bold font-mono text-slate-900 dark:text-white">{loadPercent}%</span>
        </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
            {cores.map((coreLoad, idx) => (
              <div
                key={idx}
                className="bg-slate-50 dark:bg-slate-950 p-2.5 rounded-xl border border-slate-200 dark:border-slate-800/80 flex flex-col justify-between"
              >
                <div className="flex justify-between items-center text-[11px] mb-1">
                  <span className="text-slate-500 dark:text-slate-400 font-mono">Core {idx}</span>
                  <span className="text-slate-800 dark:text-slate-200 font-mono font-semibold">{coreLoad}%</span>
                </div>
                <div className="w-full bg-slate-200 dark:bg-slate-900 h-1.5 rounded-full overflow-hidden">
                  <div
                    className={`h-full rounded-full transition-all duration-300 ${
                      coreLoad > 85 ? 'bg-rose-500' : coreLoad > 65 ? 'bg-amber-500' : 'bg-indigo-500'
                    }`}
